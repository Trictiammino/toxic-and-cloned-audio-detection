...
#
# NOTA -- VERSIONE INVERTITA:
# In questo script i ruoli di REALI e CLONATI sono invertiti rispetto alla
# versione originale: il modello (One-Class Autoencoder) viene addestrato
# sui CLONATI (classe "normale") e valutato/testato sui REALI (classe
# "anomala"). I test size restano quelli originali: 0.10 per i reali,
# 0.20 per i clonati (vedi sezione "Holdout test split" piu' sotto). Tutto
# il resto della pipeline (feature selection, calibrazione soglia, k-fold,
# grid search, ensemble, plot, salvataggio) e' rimasto invariato: viene
# solo cambiato, in main(), quale classe alimenta il fit dell'autoencoder
# e quale viene usata come classe anomala in fase di valutazione.
# ==============================================================================

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent

# --- Percorso audio reali (in questa versione: classe ANOMALA, usata solo in test) ---
CSV_ORIGINAL_PATH = (
    PROJECT_ROOT / "datasets" / "violence" / "extracted_audio_features"
    / "audio" / "features_real_audio_dataset.csv"
)

# --- Percorsi audio clonati (in questa versione: classe NORMALE, usata per il training) ---
...
# --- Percorsi output ---
OUTPUT_DIR  = PROJECT_ROOT / "classifiers" / "one_class_autoencoder" / "violence" / "voice_clone_detection" / "train_cloned_results"
...
# --- Holdout test split (invariati come richiesto: 0.10 per i reali, 0.20 per i clonati) ---
TEST_SIZE_REAL    = 0.10
...
def run_grid_search(X_real_tv: np.ndarray, cloned_tv: dict) -> dict:
    ...
    log.info(f"Train fold: solo classe normale ({LABEL_NORMAL})  |  Val fold: normale_val + anomala_val ({LABEL_ANOMALY})")
    log.info(f"{LABEL_NORMAL} train/val: {n_real}  |  {LABEL_ANOMALY} train/val (tot): {n_cloned}")
...
def run_kfold(X_real_tv: np.ndarray, cloned_tv: dict, best_pipeline=None) -> dict:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_combined, y_combined), start=1):
        ...
        log.info(
            f"  Fold {fold}: "
            f"Inlier({LABEL_NORMAL.lower()})={inlier_rate:.4f}  FPR={fpr_fold:.4f}  |  "
            f"Detect({LABEL_ANOMALY.lower()})={detection_rate:.4f}  FNR={fnr_fold:.4f}  |  "
    ...
    log.info(f"\n  Media Inlier Rate ({LABEL_NORMAL.lower()})       : {mean_inlier:.4f} +/- {std_inlier:.4f}")
    log.info(f"  Media False Positive Rate       : {mean_fpr:.4f}    +/- {std_fpr:.4f}")
    log.info(f"  Media Detection Rate ({LABEL_ANOMALY.lower()})  : {mean_det:.4f}    +/- {std_det:.4f}")
...
def train_and_evaluate(
    ...
    log.info(f"\n  [TEST SET -- {LABEL_NORMAL.upper()} (classe normale, holdout)]")
    ...
    for name, X_cloned_test in cloned_test.items():
        ...
        _save_confusion_matrix(
            cm_src, f"test_{LABEL_NORMAL.lower()}_vs_{name}",
            f"Confusion Matrix (Test Set) -- {LABEL_NORMAL} vs {name.capitalize()}"
        )
    ...
    log.info(f"\n  [TEST COMBINATO: {LABEL_NORMAL} vs {sources_str}]")
    ...
    _save_confusion_matrix(
        cm_comb, f"test_combined_all_{LABEL_ANOMALY.lower()}",
        f"Confusion Matrix (Test Set) -- {LABEL_NORMAL} vs All {LABEL_ANOMALY}"
    )
...
def save_results(
    ...
    metrics = {
        "config": {
            "model":                        "One-Class Autoencoder (v3: calibrazione soglia robusta + ensemble) -- training su CLONATI, test su REALI (inversione)",
            "training_data":                f"cloned audio train/val split ({', '.join(CLONED_SOURCES.keys())}), al netto del calibration set",
            "test_sources":                 ["real"],
            "test_size_real":               TEST_SIZE_REAL,
            "test_size_cloned":             TEST_SIZE_CLONED,
            "calibration_size":             CALIBRATION_SIZE,
    ...
    log.info(
        f"\n  ROC-AUC finale (test combinato {LABEL_NORMAL.lower()} vs all {LABEL_ANOMALY.lower()}): "
        f"{eval_results['combined_all_cloned']['roc_auc']:.4f}"
    )
    log.info(
        f"  FPR finale (test set, {LABEL_NORMAL.lower()}): "
...
def main():
    ...
    log.info("VERSIONE INVERTITA: il modello si addestra sui CLONATI (classe normale)")
    log.info("e viene valutato sui REALI (classe anomala da rilevare in test).")
    ...
    log.info(f"Training: audio clonati ({sources_str})  |  Test source: audio reali")
    ...
    # ------------------------------------------------------------------------
    # INVERSIONE REALI/CLONATI:
    # la classe "normale" su cui viene addestrato l'autoencoder e' ora
    # l'insieme dei CLONATI (qwen3 + chatterbox combinati); la classe
    # "anomala" rilevata in fase di test e' ora l'audio REALE.
    # I test size restano invariati: 0.10 per i reali (TEST_SIZE_REAL),
    # 0.20 per i clonati (TEST_SIZE_CLONED).
    # ------------------------------------------------------------------------
    X_cloned_fit_all   = np.vstack(list(cloned_fit.values()))
    X_cloned_calib_all = np.vstack(list(cloned_calib.values()))
    X_cloned_test_all  = np.vstack(list(cloned_test.values()))

    train_fit          = X_cloned_fit_all          # usato per fittare l'autoencoder (classe normale = clonati)
    train_calib         = X_cloned_calib_all        # calibrazione soglia, lato classe normale (clonati)
    train_test           = X_cloned_test_all         # holdout finale, lato classe normale (clonati) -> FPR
    anomaly_fit_dict    = {"real": X_real_fit}      # classe anomala (reali), solo per validazione/kfold/grid search
    anomaly_calib_dict  = {"real": X_real_calib}    # calibrazione soglia, lato classe anomala (reali)
    anomaly_test_dict   = {"real": X_real_test}     # holdout finale, lato classe anomala (reali) -> detection rate

    grid_search_results = None
    best_pipeline = None
    if GRID_SEARCH_ENABLED:
        grid_search_results = run_grid_search(train_fit, anomaly_fit_dict)
        best_pipeline = grid_search_results["best_pipeline"]

    kfold_results = run_kfold(train_fit, anomaly_fit_dict, best_pipeline)

    pipe, eval_results = train_and_evaluate(
        train_fit, train_test, anomaly_fit_dict, anomaly_test_dict, best_pipeline,
        X_real_calib=train_calib, cloned_calib=anomaly_calib_dict,
    )
