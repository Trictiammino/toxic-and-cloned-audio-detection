...
# --- Percorsi input ---
CSV_PATH       = PROJECT_ROOT / "generate_cloned_audio" / "chatterbox_tts" / "violence" / "extracted_audio_features" / "audio" / "features_cloned_audio_dataset.csv"
TRAIN_TSV_PATH = PROJECT_ROOT / "generate_cloned_audio" / "chatterbox_tts" / "violence" / "cloned_audio_dataset.tsv"

# --- Percorsi output ---
OUTPUT_DIR   = PROJECT_ROOT / "classifiers" / "xgboost" / "violence" / "toxic_audio_detection" / "cloned audio" / "chatterbox_tts"
...
# --- XGBoost (usati solo se GRID_SEARCH_ENABLED=False) ---
XGB_N_ESTIMATORS     = 100
XGB_MAX_DEPTH        = 6
XGB_LEARNING_RATE    = 0.1
XGB_SUBSAMPLE        = 0.8
XGB_COLSAMPLE_BYTREE = 0.8
XGB_SCALE_POS_WEIGHT = None   # None = calcolato automaticamente come neg/pos

# Calcolato in main() da neg/pos sul dataset (oppure = XGB_SCALE_POS_WEIGHT se impostato a mano)
...
# --- Pre-processing ---
SCALE_FEATURES     = False   # StandardScaler nella pipeline
...
def save_top_features(
    ...
    L = n_foglie per albero, D = profondità massima. Per XGBoost binario
    restituisce direttamente i valori SHAP per la classe positiva (tossico).

    Nota: se SCALE_FEATURES=True, i valori SHAP sono calcolati sulle feature
    scalate (quelle effettivamente viste dal modello), mentre il campo
    "value" nel JSON riporta il valore originale (non scalato), per leggibilità.
    ...
    if SCALE_FEATURES:
        X_test_model = pipe.named_steps["scaler"].transform(X_test)
    else:
        X_test_model = X_test

    log.info("  Calcolo SHAP values (TreeExplainer)...")
    explainer = shap.TreeExplainer(clf)
    # shap_values: shape (n_samples, n_features, n_classes) per modelli multi-output
    # oppure (n_samples, n_features) per classificazione binaria — gestiamo entrambi
    ...
    if isinstance(shap_values_raw, list):
        # lista [class0, class1], ciascuna (n_samples, n_features)
        sv = np.array(shap_values_raw[1])
    elif shap_values_raw.ndim == 3:
        # (n_samples, n_features, n_classes)
        sv = shap_values_raw[:, :, 1]
    else:
        # XGBoost binario: già (n_samples, n_features)
        sv = shap_values_raw
...
def train_and_evaluate(
    ...
    for split_name, X_s, y_s in [("test", X_test, y_test)]:
        ...
        log.info(f"[TEST] Accuracy={acc:.4f}, ROC-AUC={auc:.4f}, F1-macro={f1:.4f}")
...
def save_results(
    ...
    # Scaler + classifier via joblib (compatibile con script originale)
    if SCALE_FEATURES:
...
def main():
    ...
    log.info("=" * 60)
    log.info("Toxic Audio Detection — XGBoost Classifier")
    log.info("=" * 60)
    ...
    # scale_pos_weight per il bilanciamento delle classi (equivalente a class_weight="balanced")
    neg = int((y == 0).sum())
    pos = int((y == 1).sum())
    SCALE_POS_WEIGHT = XGB_SCALE_POS_WEIGHT if XGB_SCALE_POS_WEIGHT is not None else neg / pos
    log.info(f"scale_pos_weight: {SCALE_POS_WEIGHT:.4f}  (neg={neg}, pos={pos})")
